module.exports = {
  ...
  execute(bot, channel, from, text, commandNames) {
    let type = text.includes('audio') ? 'audio'
             : text.includes('tech') ? 'technical'
             : 'excuse'
    let types = {};
    types['technical'] = {
    ...
    function getJargon(jargonType) {
      if (jargonType != 'excuse' && jargonType != 'technical' && jargonType != 'audio' ) {return bot.telegram.sendMessage(channel, "I'm supposed to be writing the nonsense here... Wanna try that again?");}
      ...
      bot.telegram.sendMessage(channel, sentence);
    }
