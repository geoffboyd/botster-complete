  ...
  execute(bot, channel, from, text, commandNames) {
    ...
    */
    ...
    if (!args[0] || args[1] || !Number(args[0])) {
      return bot.telegram.sendMessage(channel, 'Tell me what number you want to test.');
    } else if ((Number(args[0]) != Math.floor(Number(args[0])) || (Number(args[0]) < 1)) ) {
      return bot.telegram.sendMessage(channel, "You didn't input a natural number.");
    } else if (Number(args[0] > 9007199254740990)){
      return bot.telegram.sendMessage(channel, "That's too big-brain for me. Try a smaller number.");
    } else {
      n = Number(args[0]);
      numberArray.push(n);
    };
    ...
    bot.telegram.sendMessage(channel, 'Number of steps: ' + numberArray.length);
//    bot.telegram.sendMessage(channel, "Your number's journey: " + numberArray.join(" "));
	if (numberArray.join(" ").length > 2000) {
		...
		bot.telegram.sendMessage(channel, "Your number's journey is attached.", attachment);
