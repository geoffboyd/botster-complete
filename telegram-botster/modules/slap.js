module.exports = {
  ...
  execute(bot, channel, from, text, commandNames) {
    ...
    if (!args[0]) {
      bot.telegram.sendMessage(channel, 'You need to tell me who you are slapping');
    } else if (args[1]){
      bot.telegram.sendMessage(channel, "Slow down, pal. Let's slap one person at a time, alright?")
    } else {
      bot.telegram.sendMessage(channel, slaps[Math.floor(Math.random()*11)]);
    }
  },
};
